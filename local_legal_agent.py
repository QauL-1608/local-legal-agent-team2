...
def process_document(uploaded_file, vector_db: Qdrant):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            knowledge = Knowledge(
                name = "Local Legal Knowledge",
                vector_db=vector_db,
                readers={"pdf": PDFReader(chunk=True)}
            )
            # Add contents asynchronously 
            asyncio.run(
                knowledge.add_contents_async(paths=[temp_dir], reader=PDFReader(chunk=True))
            )
           ...
           # knowledge.load()
            ...
            st.write("Knowledge ingested and ready for use!")

           # st.write("Verifying knowledge base...")
           # test_results = knowledge.search("test")
           # if not test_results:
...
def main():
    ...
    st.title("WAKILI M-SMART Local AI Legal Agent Team")
